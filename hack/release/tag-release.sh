...
#
# rc: refuses when HEAD already carries an rc tag (nothing new to soak) or
#     when the version has already gone GA.
# ga: refuses unless HEAD is exactly the commit of the newest rc. The same
#     invariant is enforced again by release.yaml before anything is
#     published, so a mistaken tag never reaches a registry.
#
...
if git rev-parse -q --verify "refs/tags/$ga_tag" >/dev/null; then
	fail "$ga_tag already exists: $branch is released and read-only. Hotfix = cut release-X.Y.Z+1 from main."
fi
...
rc)
	...
	;;
ga)
	[ -n "$last_rc" ] || fail "no rc tag for $version; GA must promote a soaked rc"
	rc_sha=$(git rev-parse "$last_rc^{commit}")
	[ "$rc_sha" = "$head" ] ||
		fail "HEAD of $branch (${head:0:12}) is not the commit of $last_rc (${rc_sha:0:12}); tag a new rc and soak it first"
	tag=$ga_tag
	msg="$version"
	;;
*)
