...
// sandboxResourceAlias is the suffix the standalone plugin advertises
// under as a kubelet resource ("rebellions.ai/<alias>"). Matches the
// plugin binary's defaultAlias (github.com/RBLN-SW/sandbox-device-plugin).
//
// Hardcoded — and injected into the DaemonSet as RBLN_PT_ALIAS — so
// that operators reading the rendered manifest can see exactly what
// resource name kubelet will see, without chasing it through the binary.
//
// To change this:
//  1. update the plugin binary's defaultAlias to match,
//  2. update KubeVirt permittedHostDevices.pciHostDevices[*].resourceName
//     to "rebellions.ai/<new-alias>" (otherwise admission rejects VMIs).
//
// Naming choice (npu_pt, not npu): keeps the VM-passthrough resource
// distinct from the container-side "rebellions.ai/npu", so the
// scheduler auto-partitions container Pods and VMIs by node without
// needing taints/webhooks.
const sandboxResourceAlias = "npu_pt"
		...
		WithContainers([]*corev1.Container{
			k8sutil.NewContainerBuilder().
				...
				WithEnvs([]corev1.EnvVar{
					// RBLN_PT_ALIAS controls the advertised resource
